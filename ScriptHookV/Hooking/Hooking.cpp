...
DetourHook<__int64 __fastcall(__int64, __int64)>Detour_get_event_at_index;
__int64 __fastcall HK_get_event_at_index(__int64 Cgroup, __int64 eventindex)
{
	int ret = Detour_get_event_at_index(Cgroup, eventindex);
	if (Cgroup == 1)
	{
		LOG_PRINT( "%d:%d", ret, eventindex);
	}
	return Detour_get_event_at_index(Cgroup, eventindex);
}
...
bool Hooking::HookFunctions()
{
	...
	else
	{
		...
		//	&& Detour_get_event_at_index.Hook("48 83 EC 20 4C 8D 05 ? ? ? ? 83 CB FF "_Scan.as<decltype(&HK_get_event_at_index)>(), HK_get_event_at_index,"GET_EVENT_AT_INDEX")
...
bool Hooking::UnHookFunctions()
{
	...
	else
	{
		return true
			...
			//&& Detour_get_event_at_index.UnHook()
		;
	}
}
