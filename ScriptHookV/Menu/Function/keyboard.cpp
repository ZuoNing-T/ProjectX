...
{
	//return (key < KEYS_SIZE) ? ((GetTickCount() < keyStates[key].time + MAX_DOWN) && !keyStates[key]sUpNow) : false;
...
{
	//bool b = (key < KEYS_SIZE) ? (GetTickCount() < keyStates[key].time + NOW_PERIOD && keyStates[key]sUpNow) : false;
	//if (b && exclusive)
	//	ResetKeyState(key);
	//return b;
