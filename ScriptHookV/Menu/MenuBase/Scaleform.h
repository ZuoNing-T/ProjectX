	...
	void CallFunction(std::string function, std::vector<ScaleArg> args = {}) {
		...
		for (auto arg : args) {
			if (std::holds_alternative<std::string>(arg)) {
				 ...
				 GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(std::get<int>(arg));
			}
			else if (std::holds_alternative<float>(arg)) {
				 GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_FLOAT(std::get<float>(arg));

			}
			else if (std::holds_alternative<double>(arg)) {
				 GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_FLOAT((float)std::get<double>(arg));

			}
			else if (std::holds_alternative<bool>(arg)) {
				 GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_BOOL(std::get<bool>(arg));

			}
			else if (std::holds_alternative<ScaleformArgumentTXD>(arg)) {
				 GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING((char *)std::get<ScaleformArgumentTXD>(arg).Txd().c_str());
			}
			else {
			}
		}
