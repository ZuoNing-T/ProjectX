...
public:
	Vector3 Zero() const { return Vector3(0, 0, 0); }
	Vector3 WorldUp() const { return Vector3(0, 0, 1); }
	Vector3 WorldDown() const { return Vector3(0, 0, -1); }
	float Length() const { return sqrtf((pow(x, 2) + pow(y, 2) + pow(z, 2))); };
};
